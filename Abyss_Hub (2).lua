...
local function selectWeapon()
    pcall(function()
        ...
        local char = player.Character
        if not char then return end
        -- Se já tem a tool certa equipada, não faz nada
        local equipped = char:FindFirstChildOfClass("Tool")
        if equipped and (equipped.ToolTip == _G.ChooseWP or equipped.Name == _G.ChooseWP) then return end
        -- Procura na mochila por ToolTip ou por nome
        for _, v in pairs(player.Backpack:GetChildren()) do
            if v:IsA("Tool") and (v.ToolTip == _G.ChooseWP or v.Name == _G.ChooseWP) then
                _G.SelectWeapon = v.Name
                char.Humanoid:EquipTool(v)
                return
            end
        end
        -- Fallback: equipa qualquer tool disponível
        local any = player.Backpack:FindFirstChildOfClass("Tool")
        if any and char.Humanoid then
            char.Humanoid:EquipTool(any)
        end
    end)
end
...
local function AttackEnemy(enemy)
    ...
    if enemy and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
        pcall(function()
            ...
            -- Damage is handled by the FastAttack Heartbeat loop via RegisterAttack/RegisterHit
        end)
    end
end
...
RunService.Heartbeat:Connect(function()
    ...
    -- Ensure weapon is equipped every frame when farming
