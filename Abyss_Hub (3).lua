...
local function selectWeapon()
    pcall(function()
        ...
        if not char then return end
        local currentTool = char:FindFirstChildOfClass("Tool")
        if currentTool and currentTool.ToolTip == _G.ChooseWP then return end
...
local _attackInterval = 0.15

local function AttackEnemy(enemy)
    if getgenv().BuddhaTransforming then return end
    if getgenv().FastAttack then return end
    local now = tick()
    if now - _lastAttackTime < _attackInterval then return end
...
local function QuestNeta()
    ...
    if World1 then
        ...
        else                Mon="Galley Captain";    Qdata=2; Qname="FountainQuest";  NameMon="Galley Captain";    PosQ=CFrame.new(5259.81982,37.3500175,4050.0293,.087131381,0,.996196866,0,1,0,-0.996196866,0,.087131381); PosM=CFrame.new(5441.9516601562,42.502059936523,4950.09375)
        end
    elseif World2 then
        ...
        else                Mon="Water Fighter";    Qdata=2; Qname="ForgottenQuest"; NameMon="Water Fighter";     PosQ=CFrame.new(-3054.44458,235.544281,-10142.8193,.990270376,0,-0.13915664,0,1,0,.13915664,0,.990270376);       PosM=CFrame.new(-3352.9013671875,285.01556396484,-10534.841796875)
        end
    elseif World3 then
...
function startBoneFarm()
    ...
    if not World3 then
        Window:Notify({Title="Unavailable", Text="Bone Farm only works in Sea 3!", Duration=4})
        getgenv().FarmBone = false
        return
    end
...
function startKataFarm()
    ...
    if not World3 then
        Window:Notify({Title="Unavailable", Text="Kata Farm only works in Sea 3!", Duration=4})
        getgenv().FarmKata = false
        return
    end
...
function startAuraFarm()
    ...
    auraFarmConn = RunService.Heartbeat:Connect(function(dt)
        ...
        if enemy then
            ...
            getgenv().CurrentTargetMob = nil
            local auraFallback
            if World1 then     auraFallback = CFrame.new(-1141,14,3831)
            elseif World2 then auraFallback = CFrame.new(-429,71,1836)
            else               auraFallback = CFrame.new(-246,47,5584) end
            TweenToFixedPos(root, auraFallback, getgenv().FlySpeed)
        end
    end)
end
...
function startPhaBinhFarm()
    ...
    if not World3 then
        Window:Notify({Title="Unavailable", Text="Pha Binh Farm only works in Sea 3!", Duration=4})
        getgenv().FarmPhaBinh = false
        return
    end
...
function startTyrantFarm()
    ...
    if not World3 then
        Window:Notify({Title="Unavailable", Text="Tyrant Farm only works in Sea 3!", Duration=4})
        getgenv().FarmTyrant = false
        return
    end
