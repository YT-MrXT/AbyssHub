...
-- Atualiza distância dos players a cada 0.5s (leve)
spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if not getgenv().ESPPlayers then clearESPPlayers() return end
            -- Remove bills de jogadores que saíram
...
-- Atualiza frutas a cada 2s (frutas não se movem)
spawn(function()
...
-- Auto Storage Fruit
getgenv().AutoStorageFruit = false
Tabs.Roll:AddToggle({
    Name = "Auto Storage Fruit",
    Default = false,
    Callback = function(v)
        getgenv().AutoStorageFruit = v
    end
})
spawn(function()
    while task.wait(0.5) do
        pcall(function()
            if getgenv().AutoStorageFruit then
                local backpack = player:FindFirstChild("Backpack")
                local character = player.Character
                local function storeFruit(fruitTool)
                    local fruitName = fruitTool.Name
                    ReplicatedStorage.Remotes.CommF_:InvokeServer("StoreFruit", fruitName, fruitTool)
                end
                if backpack then
                    for _, item in ipairs(backpack:GetChildren()) do
                        if item:IsA("Tool") and (string.find(item.Name, "Fruit") or string.find(item.Name, "Fruta")) then
                            storeFruit(item)
                            task.wait(0.2)
                        end
                    end
