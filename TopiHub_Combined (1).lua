...
local Tabs = {
    Settings  = Window:CreateTab({Name="Settings",  Title="Settings",       Subtitle="Configurar",                 Icon="rbxassetid://"}),
    ...
    Status    = Window:CreateTab({Name="Status",    Title="Status",         Subtitle="Fruit / Eyes / Buso",         Icon="rbxassetid://"}),
    FarmOther = Window:CreateTab({Name="FarmOther", Title="Farming Other",  Subtitle="Em breve",                    Icon="rbxassetid://"}),
...
local function ResumeNormalFarm()
    ...
    getgenv().NormalPaused = false
    if     getgenv().FarmLevel  and SelectedFarm == "Farm cấp"          then startLevelFarm()
...
Tabs.Main:AddToggle({
    ...
    Callback=function(v)
        ...
        if SelectedFarm=="Farm cấp"           then getgenv().FarmLevel=v
        ...
        if v then
            startFly()
            if SelectedFarm=="Farm cấp"           then startLevelFarm()
...
Tabs.Stats:AddSlider({
    Name="Stats Value", Min=0, Max=1000, Default=10,
...
for _, s in ipairs(statTypes) do
    ...
    spawn(function()
        while task.wait(Sec) do
            pcall(function()
                if getgenv()[flagKey] then statsSetings(statKey, getgenv().pSats) end
            end)
        end
    end)
end
...
local fruitLabel = Tabs.Status:AddLabel("Devil Fruit: carregando...")
local eyesLabel  = Tabs.Status:AddLabel("Eyes: carregando...")
local busoLabel  = Tabs.Status:AddLabel("Buso: carregando...")
...
spawn(function()
    while task.wait(2) do
        pcall(function()
            ...
            local hasFruit = (lp.Character and lp.Character:FindFirstChild(fruitName))
                          or lp.Backpack:FindFirstChild(fruitName)
            if fruitLabel and fruitLabel.Label then
                fruitLabel.Label.Text = "Devil Fruit: " .. (fruitName ~= "" and fruitName or "Nenhuma")
                    .. (hasFruit and " [Equipada]" or " [Nao equipada]")
            end

            -- Eyes (Instinct)
            local hasEyes = lp.Character and (
                lp.Character:FindFirstChild("HasInstinct") or
                lp.Character:FindFirstChild("Instinct")
            )
            if eyesLabel and eyesLabel.Label then
                eyesLabel.Label.Text = "Eyes: " .. (hasEyes and "Ativo" or "Inativo")
            end

            -- Buso
            local hasBuso = lp.Character and (
                lp.Character:FindFirstChild("HasBuso") or
                lp.Character:FindFirstChild("Buso")
            )
            if busoLabel and busoLabel.Label then
                busoLabel.Label.Text = "Buso: " .. (hasBuso and "Ativo" or "Inativo")
            end
        end)
    end
end)
...
Tabs.FarmOther:AddLabel("Em desenvolvimento — em breve aqui.")
Tabs.FarmOther:AddLabel("Esta aba ainda nao tem funcoes ativas.")
...
Window:Notify({
    Title   = "Topi Hub",
    Text    = "Script carregado! Todas as funcionalidades prontas.",
    Duration = 5
})
