    ...
    while task.wait(1) do
        pcall(function()
            if getgenv().AutoRollFruit then
                ...
                local response = gachaRemote:InvokeServer("Cousin", "Buy")
                if response then
                    Window:Notify({Title="Roll Fruit", Text="Fruit rolled!", Duration=2})
                else
                    Window:Notify({Title="Roll Fruit", Text="Failed: no money or cooldown.", Duration=2})
                end
            end
        end)
    end
    ...
    while task.wait(1) do
        pcall(function()
            if getgenv().AutoRollBones then
                ...
                local response = mainRemote:InvokeServer("Bones", "Buy", 1)
                if response then
                    Window:Notify({Title="Roll Bones", Text="Bones rolled!", Duration=2})
                else
                    Window:Notify({Title="Roll Bones", Text="Failed: no bones or daily limit reached.", Duration=2})
                end
            end
        end)
    end
