...
local function GetFarmCFrame(mob)
    ...
    if _G.ChooseWP == "Blox Fruit" then
        targetPos = Vector3.new(mobPos.X, mobPos.Y + 5, mobPos.Z)
    else
        local offset = -lookFlat * 10
        targetPos = Vector3.new(mobPos.X + offset.X, mobPos.Y + 5, mobPos.Z + offset.Z)
    end
...
local FastAttackConfig = {RANGE=150, ATTACKS_PER_FRAME=10, HITS_PER_FRAME=10, FRUIT_SPAM=10}
