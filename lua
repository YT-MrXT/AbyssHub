...
local FastAttackConfig = {RANGE=300, ATTACKS_PER_FRAME=10, HITS_PER_FRAME=10, FRUIT_SPAM=10}
...
Tabs.Main:AddSection("Mastery Farm")

getgenv().MasteryWeapon = "Melee"
Tabs.Main:AddDropdown({
    Name="Mastery Weapon",
    Options={"Melee","Sword","Blox Fruit","Gun"},
    Default="Melee",
    Callback=function(v) getgenv().MasteryWeapon=v end
})
...
Tabs.Main:AddToggle({
    ...
    Default=false,
    Callback=function(v)
        getgenv().AutoMastery = v
        if v then
            _G.ChooseWP = "Melee"
            getgenv().IsFarming=true; getgenv().Noclip=true; getgenv().AutoBusoLoop=true
            getgenv().FarmLevel=true
            startFly(); startLevelFarm()
        else
            getgenv().IsFarming=false; getgenv().Noclip=false; getgenv().AutoBusoLoop=false
            getgenv().FarmLevel=false
            stopFly(); stopLevelFarm()
        end
    end
})

-- Mastery skill loop: uses Melee to deal damage, fires skills of chosen weapon at low HP
spawn(function()
    while task.wait(0.2) do
        pcall(function()
            if not getgenv().AutoMastery then return end
            local mob = getgenv().CurrentTargetMob
                ...
                for _, item in pairs(player.Backpack:GetChildren()) do
                    if item:IsA("Tool") and item.ToolTip == weapon then
                        ...
                        for _, key in ipairs(skills) do
                            if not getgenv().AutoMastery then break end
